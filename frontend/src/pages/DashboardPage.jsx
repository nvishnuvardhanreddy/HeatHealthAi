...
export function DashboardPage({ authority = false }) {
  ...
  useEffect(() => {
    ...
    }, () => setError('Location permission was denied. Showing the nearest available demo data. Grant location access for your city\'s live heat risk.'));
  }, [authority]);
  const detectLocation = () => { if (!navigator.geolocation) return; navigator.geolocation.getCurrentPosition(async ({ coords }) => { const nextLocation = { latitude: coords.latitude, longitude: coords.longitude, accuracy: coords.accuracy }; setLocation(nextLocation); try { await gisService.updateLocation(coords.latitude, coords.longitude, coords.accuracy); } catch (_) { /* Keep the dashboard usable if location storage is unavailable. */ } loadDashboard(nextLocation); }, () => setError('Location access is disabled. Enable it in browser settings to see live heat risk for your location.')); };
